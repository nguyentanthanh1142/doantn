...
<body>
				...
				<div class="col-12">
					<ul class="breadcrumb">
						...
						<li class="breadcrumb-item active"><a
							href="<c:url value="${slug}?page=1"/>"><strong><span>${categoryEntity.name}
										${manuEntity.manufacturer_name} </span></strong></a></li>
					</ul>
				</div>
			</div>
		</div>
		...
		<div class="container">
			<div
				...
				<div class="sort-cate clearfix shadow-sm">
					<div class="row">
						<div class="col-sm-6 col-12">
							<c:if test="${prodManuCatePaginate.size()>0}">
							...
							<form class="form-inline form-viewpro order_bar">
								<div class="form-group" style="margin: 0;">
									<select class="form-control"
										...
										<option
											value="<c:url value="${slug}?page=1&order=desc&sortBy=ctime"/>"  ${sort =='ctime' && order == 'desc'?'selected':''  }>Mới
											nhất</option>
										<option
											value="<c:url value="${slug}?page=1&order=asc&sortBy=ctime"/>" ${sort =='ctime' && order == 'asc'?'selected':''  }>Cũ
											đến mới</option>
										<option
											value="<c:url value="${slug}?page=1&order=asc&sortBy=price"/>" ${sort =='price' && order == 'asc'?'selected':''  }>Giá
											tăng dần</option>
										<option
											value="<c:url value="${slug}?page=1&order=desc&sortBy=price"/>" ${sort =='price' && order == 'desc'?'selected':''  }>Giá
											giảm dần</option>
										<option
											value="<c:url value="${slug}?page=1&order=asc&sortBy=alpha"/>" ${sort =='alpha' && order == 'asc'?'selected':''  }>Từ
											A->Z</option>
										<option
											value="<c:url value="${slug}?page=1&order=desc&sortBy=alpha"/>" ${sort =='alpha' && order == 'desc'?'selected':''  }>Từ
											Z->A</option>
									</select>
								</div>
							</form>
				...
				<div class="product-item row">
					<c:if test="${prodManuCatePaginate.size()>0 }">
						<c:forEach var="item" items="${prodManuCatePaginate}"
							varStatus="index">
						...
						<!--<c:forEach var="item" begin="1" end="${paginateInfo.totalPage}"
							...
							<c:if test="${(loop.index)==paginateInfo.currentPage}">
								<li class="page-item active"><a class="page-link"
									href="<c:url value="/${slug}?${rppart1 }${loop.index}${ rppart2}"/>">${loop.index}</a></li>
							</c:if>
							<c:if test="${(loop.index) != paginateInfo.currentPage}">
	...
	$(function () {
	    ...
	    function showPage(whichPage) {
	        ...
	        getPageList(totalPages, currentPage, paginationSize).forEach( item => {
	            $("<li>").addClass("page-item")
	                ...
	                $("<a>").addClass("page-link").attr({
	                    href: '<c:url value="/${slug}?${rppart1 }"/>' + item +"${rppart2 }"}).text(item || "...")
	    ...
	    $("#next-page").on("click", function () {
	    	...
	    	{
	    		return window.location = '<c:url value="/${slug}?${rppart1 }"/>' + (currentPage<totalPages?currentPage+1:currentPage) + "${rppart2}";
	    ...
	    $("#previous-page").on("click", function () {
	    	if(currentPage>1)
	    	{
	    		return window.location = '<c:url value="/${slug}?${rppart1 }"/>' + (currentPage>1?currentPage-1:currentPage)+ "${rppart2}";
	    	}
	    	else return;
	    });
	});
