		...
		<div class="container">
			<div
						...
						<c:otherwise>
							<div class="row">
								...
								<div class="col-sm-6 col-12" style="margin-top: 5px;">
									<h3>Tìm thấy ${totalData} kết quả với từ khóa ${q }</h3>
								</div>
									...
									<form class="form-inline form-viewpro order_bar">
										<div class="form-group" style="margin: 0;">
											<select class="form-control"
												...
												<option
													value="<c:url value="/search?q=${q}&currentPage=1&sortBy=ctime&order=desc"/>" ${sort =='ctime' && order == 'desc'?'selected':''  }>Mới
													nhất</option>
												<option
													value="<c:url value="/search?q=${q}&currentPage=1&sortBy=ctime&order=asc"/>" ${sort =='ctime' && order == 'asc'?'selected':''  }>Cũ
													đến mới</option>
												<option
													value="<c:url value="/search?q=${q}&currentPage=1&sortBy=price&order=asc"/>" ${sort =='price' && order == 'asc'?'selected':''  }>Giá
													tăng dần</option>
												<option
													value="<c:url value="/search?q=${q}&currentPage=1&sortBy=price&order=desc"/>" ${sort =='price' && order == 'desc'?'selected':''  }>Giá
													giảm dần</option>
												<option
													value="<c:url value="/search?q=${q}&currentPage=1&sortBy=alpha&order=asc"/>" ${sort =='alpha' && order == 'asc'?'selected':''  }>Từ
													A->Z</option>
												<option
													value="<c:url value="/search?q=${q}&currentPage=1&sortBy=alpha&order=desc"/>" ${sort =='alpha' && order == 'desc'?'selected':''  }>Từ
													Z->A</option>
				...
				<div class="product-item row">
					<c:forEach var="item" items="${searchPaginate}" varStatus="index">
