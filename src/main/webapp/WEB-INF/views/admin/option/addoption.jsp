...
<body>
	<c:url var="save" value="/quan-tri/tuy-chon/save" />
	<c:url var="list" value="/quan-tri/tuy-chon" />
	<form:form modelAttribute="option" action="${save}"
							...
							<div class="col-md-9">
								<div class="form-group">
									...
									<c:if test="${opgroup != null }">
										<form:select name="opgroup" class="form-control"
											path="optiongroup_id">
											<c:forEach var="item" items="${opgroup}">
												<form:option value="${item.optiongroups_id}"
													label="${item.optiongroupname}"></form:option>
											</c:forEach>
										</form:select>
									</c:if>
								</div>
								<div class="form-group">
									...
									<form:input class="form-control" type="text" path="optionname"
										required="required"
										value="${ empty oldvalue.optionname ? '': oldvalue.optionname }" />
