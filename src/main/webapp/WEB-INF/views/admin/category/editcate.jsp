...
<body>
	<c:url var="editsave" value="/quan-tri/danh-muc/editsave" />
	<c:url var="list" value="/quan-tri/danh-muc" />
	<form:form modelAttribute="category" action="${editsave}" method="POST"
							...
							<div class="col-md-9">
								...
								<div class="form-group">
									...
									<form:input class="form-control" type="text" path="name"
										required="required"
										value="${ empty oldvalue.name ? cateitem.name: oldvalue.name }" />
								...
								<div class="form-group">
									...
									<textarea class="form-control" id="metadesc" name="metadesc"
										required="required">${ empty oldvalue.metadesc ? cateitem.metadesc:oldvalue.metadesc}</textarea>
								</div>
								<div class="form-group">
									<label for="">Từ khóa</label>
									<form:input class="form-control" type="text"
										placeholder="Nhập tên" path="metakey" required="required"
										value="${ empty oldvalue.metakey ? cateitem.metakey: oldvalue.metakey }" />
								</div>
							</div>
							<div class="col-md-3">
								<div class="form-group">
									...
									<form:input class="form-control" type="text" path="slug"
										required="required"
										value="${ empty oldvalue.slug ? cateitem.slug: oldvalue.slug }" />
									<c:if test="${ not empty msgSlug}">
										<span class="form-error">${msgSlug}</span>
									</c:if>
								</div>
								<div class="form-group">
									<label>Trạng thái</label> <input type="hidden" id="status1"
										value="${cateitem.status}">
